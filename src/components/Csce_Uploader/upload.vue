<template>
  <div>
    <el-row :gutter="20" style="width: 100%;margin-left:0px">
      <el-col :span="14" style="padding-left:0px">
        <div class="fileList">
          ...
          <p class="flie" v-for="(item,index) in fileLists">{{item.F_FileName}}、</p>
          <!-- </div> -->
        </div>
      </el-col>
      <el-col :span="10">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item style="line-height:32px">
            <el-button v-if="!disabled" type="primary" @click="uploader" size="small" :disabled="disabled">上传
            </el-button>
          </el-form-item>
          <el-form-item style="line-height:32px">
            <el-button type="warning" @click="dowloader" size="small" class="csceshow">下载</el-button>
          </el-form-item>
          <el-form-item style="line-height:32px">
            <el-button type="success" @click="preview(fileLists[0])" size="small" class="csceshow">预览</el-button>
          </el-form-item>
        </el-form>
      </el-col>
    </el-row>
    ...
    <div v-if="uploadshow || previewshow">
        ...
        <div>
          <div class="preview">
            ...
            <div style="height:calc(100% - 45px);position:relative">
              ...
              <video v-else-if="previewtype == 'vedio'">
                <source :src="previewFlie" type="video/mp4">您的浏览器不支持 video 标签。</video>
      ...
      <div v-if="uploadshow">
        <uploaders class="uploadertext" v-on:listenCloseClick="hide" :clickname="clickname" :fileLists="fileLists"
          v-on:fileListchange="fileListchange" :multiple="multiple" :limit="limit" :acceptlist="acceptlist"
          :keyValue="keyValue" v-on:listenpreviewClick="previewone" :fileType="fileType" :companyName="companyName">
        </uploaders>
      </div>
    </div>
  </div>
...
<script>
  import uploaders from "@/components/Csce_Uploader/uploaders";
    ...
    methods: {
      ...
      preview(item) {
        ...
        if (this.fileLists.length == 0) {
          ...
          axios
            .get(this.apiroute + "//csce/learun/adms/annexes/preview?data=" + item.F_Id)
            .then(res => {
              ...
              if (item.F_FileType == 'MP4') {
                this.previewtype = 'vedio'
              } else {
                this.previewtype = res.headers[keyArr[keyindex]].split("/")[0]
              }
            })
