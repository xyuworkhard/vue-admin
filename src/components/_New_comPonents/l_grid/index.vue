<template>
  <div class="editGrid" :class="{ isEditAble: false || !gridConfig.isEdit }" :style="{ width: gridConfig.width + 'px' }"
    v-if="isFresh">
    <el-table border ref="multipleTable" :data="getTableData" tooltip-effect="dark"
      @selection-change="handleSelectionChange" class="tabBorder" :height="gridConfig.height || 185">
      <el-table-column type="index" width="40" align="center" fixed="left"></el-table-column>
      <el-table-column type="selection" width="50" align="center" fixed="left"></el-table-column>
      <el-table-column v-for="(item, index) in gridConfig.headData" :key="index" :label="item.label" :width="item.width"
          ...
          <el-form>
            <el-form-item :prop="
              gridConfig.preVerify
                ...
                : ''
            " :rules="ruleChecked ? ruleChecked[item.name] : {}" label-width="0px" style="margin-bottom: 0;">
              <div :class="{
                ...
                <el-input v-model="scope.row[item.name]" v-if="!item.type || 'input' == item.type"
                  :title="scope.row[item.name]" placeholder="请输入"></el-input>
                <div v-else>
                  <el-date-picker v-if="'date' == item.type" v-model="scope.row[item.name]" type="date"
                    placeholder="请选择" value-format="yyyy-MM-dd" prefix-icon="icon" clear-icon="icon"
                    :title="scope.row[item.name]">
                  </el-date-picker>
                  <selectFilter :filterFiled="item.typeData.filterFiled" :dicField="item.typeData.dicField"
                    :defaultUrl="item.typeData.defaultUrl" v-model="scope.row[item.name]"
                    v-else-if="'selectFilter' == item.type" :defoption="scope.row[item.name]"></selectFilter>
                  <Select v-else-if="'select' == item.type" :showcontent="item.typeData.showcontent"
                    v-model="scope.row[item.name]" :action="item.typeData.action" :actionUrl="item.typeData.actionurl"
                    :multiple="item.typeData.multiple" :filterable="item.typeData.filterable"
                    :defoption="scope.row[item.name]" :isTags="item.typeData.isTags"></Select>
                  <Check v-else-if="'radio' == item.type" :dicType="item.typeData.dicType"
                    :dictkey="item.typeData.dictkey" v-model="scope.row[item.name]" :action="item.typeData.action"
                    :chktype="item.typeData.chktype" :portrait="item.typeData.portrait"
                    :defoption="item.typeData.defoption" :iconType="item.typeData.iconType"></Check>
                  <Check v-else-if="'checkBox' == item.type" :dicType="item.typeData.dictType"
                    :dictkey="item.typeData.dictkey" v-model="scope.row[item.name]" :action="item.typeData.action"
                    :chktype="item.typeData.chktype" :portrait="item.typeData.portrait"
                    :defoption="item.typeData.defoption" :iconType="item.typeData.iconType"></Check>
                ...
                v-model="scope.row[item.name]" :key-value="item.name+scope.$index" v-else-if="'upLoader'==item.type" />-->
                  <upLoader :file-type="item.typeData.fileType" :company-name="item.typeData.companyName"
                    :key-value="scope.row[item.name]" v-else-if="'upLoader' == item.type" />
                </div>
              </div>
            </el-form-item>
          </el-form>
    ...
    <div class="spanBtn">
      ...
      <!-- <el-button @click="addRowData" icon="el-icon-plus" type="primary" size="small"></el-button>
      <el-button @click="clearRowSelection" icon="el-icon-minus" type="primary" size="small"></el-button>-->
    </div>
  </div>
</template>

<script>
  import Select from "@/components/_New_comPonents/l_select";
  import Check from "@/components/_New_comPonents/l_checkbox";
  import upLoader from "@/components/_New_comPonents/l_upload";
  import selectFilter from "@/components/_New_comPonents/l_treeFilter";
