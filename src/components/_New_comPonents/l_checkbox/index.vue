<template>
  <div>
    <el-checkbox-group v-model="chooseItem" v-if="chktype == 'multi'"
      :class="iconType.length != 0 ? 'removeDefalut' : ''" @change="change">
      <el-checkbox v-for="(item, index) in options" :label="item.value" :key="index" :style="{
          display: portrait ? 'block' : 'inline-block',
          textAlign: portrait ? 'left' : ''
        }">
        <i v-if="iconType.length != 0" :class="chooseItem.includes(item.value) ? iconType[1] : iconType[0]"
          style="margin-right:5px"></i>
        {{ item.label }}
      </el-checkbox>
    </el-checkbox-group>
    <el-radio-group v-model="chooseItem1" v-else :class="iconType.length != 0 ? 'removeDefalut' : ''" @change="change">
      <el-radio v-for="(item, index) in options" :label="item.value" :key="index" :style="{
          display: portrait ? 'block' : 'inline-block',
          textAlign: portrait ? 'left' : ''
        }">
        <i v-if="iconType.length != 0" :class="chooseItem1.includes(item.value) ? iconType[1] : iconType[0]"
          style="margin-right:5px"></i>
        {{ item.label }}
      </el-radio>
    </el-radio-group>
  ...
  export default {
    ...
    data() {
      return {
        ...
        chooseItem: [], //复选框选中的值
        chooseItem1: '', //单选框选中的值
        dataitemUrl: '/learun/adms/dataitem/details?data=', //数据字典url
        datasourceUrl: '/learun/adms/datasource/map?data=', //数据源url
      };
    },
    created() {
      if (this.dicType) {
        let dataList = this.$store.getters.dataDicCode[this.dicType]; //store里存的数据字典数据
        if (dataList) { //如果store里有对应数据 就使用store的数据
        ...
        } else { //store没有对应数据 从接口获取
          let queryUrl = null;
          //判断数据接口类型
          if (this.action == '0') { //数据字典
            queryUrl = this.dataitemUrl + this.dicType;
          } else if (this.action == '1') { //数据源
