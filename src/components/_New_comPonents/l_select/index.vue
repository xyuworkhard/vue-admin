<template>
  <div>
    <el-select v-model="chooseItem" @change="change" size="mini" :multiple="multiple" :placeholder="'请选择'"
      :filterable="filterable" :disabled="readonly" :collapse-tags="isTags">
  ...
  export default {
    ...
    created() {
      if (this.action == 0) {
        ...
        if (this.showcontent.data == 'csce_companyca_bank') { //金融机构
        ...
        } else if (this.showcontent.data == 'csce_companyca') { //企业名称
          for (var i = 0; i < arr.length; i++) {
